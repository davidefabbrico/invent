...
List bodyMCMC(arma::vec y, int p, int nobs, arma::vec cd, arma::vec d, arma::mat X_l, arma::mat X_nl, arma::mat X_val_l, arma::mat X_val_nl, arma::vec hyperpar, arma::vec mht, int n_cat, int iter, int burnin, int thin, int ha, bool detail = false, bool pb = true) {
  ...
  arma::mat gamma_star_nl(nlp,nlp);
  for (int j = 0; j<nlp; j++) {
  ...
  arma::vec gamma_0_nl(nlp);
  for (int j = 0; j<nlp; j++) {
    gamma_0_nl(j) = hyperpar(4);
  }
  // tau star linear
      ...
      for (int k = (j+1); k<p; k++) {
        if (ha == 1) {
          if ((gamma_0_l(j) != hyperpar(4)) || (gamma_0_l(k) != hyperpar(4))) {
            ...
            eta_pl_tmp = compLinPred(nobs, eta0, X_l, beta_l_tmp, X_nl, beta_nl);
      ...
      for (int k = (j+1); k<nlp; k++) {
        ...
        if (ha == 1) {
          if ((gamma_0_l(j) != hyperpar(4)) || (gamma_0_l(k) != hyperpar(4))) {
            ...
            // new linear predictor WITH the proposed alpha star
            eta_pl_tmp = compLinPred(nobs, eta0, X_l, beta_l, X_nl, beta_nl_tmp);
