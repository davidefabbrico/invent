...
## How to Install the Package and First use
...
## `invParMCMC` - Parallel MCMC Implementation
...
#### Basic Usage
```{r, eval=TRUE}
res0 <- invParMCMC(y = y, x = x, iter = iterations, PredOutOfSample = TRUE, 
                burnin = burnin, mht = c(0.8, 0.2, 0.2, 0.6, 0.4, 0.4, 3, 1.4), thin = thin,
                ha = 2, hyperpar = c(5, 25, 3, 1, 0.00025, 0.4, 1.6, 0.2, 1.8, 0.4, 1.6, 0.2, 1.8), 
...
- Validates requested number of chains
- Uses `doParallel` and `doRNG` for reproducible parallel computation

### Output Interpretation

The function provides:
1. Progress messages during execution
2. Color-coded convergence reports
3. Detailed parameter-specific diagnostics
4. Actionable recommendations for problematic parameters
